...
import ancaPortret from "./assets/anca-portret.jpg";
...
export default function AuditClaritate() {
  ...
  const styles = {
    ...
    h1: { fontSize: 24, fontWeight: 700, fontFamily: "var(--font-serif)", color: "var(--color-brand-navy)", marginBottom: 8, lineHeight: 1.35 },
    h2: { fontSize: 20, fontWeight: 700, fontFamily: "var(--font-serif)", color: "var(--color-brand-navy)", marginBottom: 8 },
    body: { fontSize: 14, color: "var(--color-text-secondary)", lineHeight: 1.7, marginBottom: 16 },
    card: { background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "1rem 1.25rem", marginBottom: 10, boxShadow: "0 1px 6px rgba(43,45,110,0.06)" },
    btn: { background: "var(--color-background-primary)", border: "1px solid var(--color-brand-lavender)", borderRadius: "var(--border-radius-md)", padding: "10px 24px", fontSize: 14, cursor: "pointer", color: "var(--color-brand-navy)", fontWeight: 500 },
    btnPrimary: { background: "var(--color-brand-orange)", border: "none", borderRadius: "var(--border-radius-md)", padding: "12px 28px", fontSize: 14, cursor: "pointer", color: "#fff", fontWeight: 600, letterSpacing: "0.01em" },
    progress: { display: "flex", gap: 4, marginBottom: 24 },
    dot: (active, done) => ({
      height: 4, flex: 1, borderRadius: 2,
      background: done ? "var(--color-brand-orange)" : active ? "var(--color-brand-navy)" : "var(--color-border-tertiary)",
    ...
    textarea: { width: "100%", border: "1px solid var(--color-brand-lavender)", borderRadius: "var(--border-radius-md)", padding: "10px 12px", fontSize: 13, color: "var(--color-text-primary)", background: "var(--color-background-primary)", resize: "vertical", minHeight: 72, fontFamily: "var(--font-sans)", lineHeight: 1.6, boxSizing: "border-box" },
    input: { width: "100%", border: "1px solid var(--color-brand-lavender)", borderRadius: "var(--border-radius-md)", padding: "10px 12px", fontSize: 13, color: "var(--color-text-primary)", background: "var(--color-background-primary)", fontFamily: "var(--font-sans)", boxSizing: "border-box" },
  };
  ...
  if (current === "intro") return (
    <div style={styles.wrap}>
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--color-brand-orange)", marginBottom: 12 }}>Roata vieții · 10 minute</div>
        ...
        <div style={{ ...styles.card, display: "flex", gap: 20, alignItems: "center" }}>
          <img
            src={ancaPortret}
            alt="Anca Ungureanu"
            style={{ width: 88, height: 110, borderRadius: 12, objectFit: "cover", flexShrink: 0, objectPosition: "top" }}
          />
          <div>
            <div style={{ fontFamily: "var(--font-serif)", fontSize: 16, fontWeight: 700, color: "var(--color-brand-navy)", marginBottom: 6 }}>Bună, sunt Anca.</div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 600, color: "var(--color-brand-navy)" }}>{area.label}</span>
              </div>
  ...
  if (current === "results") return (
    <div style={styles.wrap}>
      ...
      <p style={styles.body}>Scorul tău mediu: <strong style={{ color: scoreColor(avg), fontWeight: 600 }}>{avg}/10</strong> — {avg <= 4 ? "există arii critice care cer atenție imediată." : avg <= 6 ? "există dezechilibre clare care îți consumă energia." : "ești la un nivel decent, dar ceva tot nu se leagă."}</p>
      ...
      <div style={{ height: 280, marginBottom: 16 }}>
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={radarData} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
            ...
            <Radar dataKey="score" stroke="var(--color-brand-navy)" fill="var(--color-brand-navy)" fillOpacity={0.12} strokeWidth={2} />
          </RadarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div style={{ background: "rgba(43,45,110,0.06)", borderRadius: "var(--border-radius-lg)", padding: "1rem 1.25rem", marginBottom: 16, borderLeft: "3px solid var(--color-brand-navy)" }}>
        <div style={{ fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--color-brand-navy)", marginBottom: 6 }}>Ariile cu cel mai mare impact acum</div>
        {lowest.map(a => (
          <div key={a.key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 13, color: "var(--color-brand-dark)" }}>{a.label}</span>
          </div>
        ))}
        <p style={{ fontSize: 12, color: "var(--color-brand-navy)", margin: "8px 0 0", lineHeight: 1.6 }}>Acestea sunt ariile unde o schimbare mică produce cel mai mare efect în restul roții.</p>
      </div>
    ...
    <div style={styles.wrap}>
      ...
      {[
        ...
        <div key={key} style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: "var(--color-brand-navy)", marginBottom: 4 }}>{q}</div>
  ...
  if (current === "action") return (
    <div style={styles.wrap}>
      ...
      <p style={styles.body}>Acum transformi ce ai descoperit în ceva concret. Unul singur din fiecare, nu o listă, nu un plan mare. <strong style={{ color: "var(--color-brand-navy)", fontWeight: 600 }}>Unul.</strong></p>

      {[
        { key: "focus", label: "O arie de focus", hint: "Pe care arie din roată îți concentrezi energia săptămâna asta?", placeholder: `Ex: ${lowest[0]?.label || "Sănătate"}` },
        ...
        <div key={key} style={{ ...styles.card, marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "var(--color-brand-navy)", marginBottom: 3 }}>{label}</div>
  ...
  if (current === "cta") return (
    <div style={styles.wrap}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <div style={{ fontSize: 32, marginBottom: 12, color: "var(--color-brand-orange)" }}>✓</div>
        <h2 style={{ ...styles.h2, textAlign: "center" }}>Ai terminat exercițiul.</h2>
        <p style={{ ...styles.body, textAlign: "center" }}>
          Scor mediu: <strong style={{ color: scoreColor(avg), fontWeight: 600 }}>{avg}/10</strong><br />
          Aria de focus aleasă: <strong style={{ color: "var(--color-brand-navy)", fontWeight: 600 }}>{action.focus || lowest[0]?.label || "—"}</strong>
        </p>
      </div>
      ...
      {/* Card Anca în CTA */}
      <div style={{ ...styles.card, display: "flex", gap: 16, alignItems: "center", marginBottom: 20 }}>
        <img
          src={ancaPortret}
          alt="Anca Ungureanu"
          style={{ width: 64, height: 80, borderRadius: 10, objectFit: "cover", flexShrink: 0, objectPosition: "top" }}
        />
        <div>
          <div style={{ fontFamily: "var(--font-serif)", fontSize: 15, fontWeight: 700, color: "var(--color-brand-navy)", marginBottom: 4 }}>Anca Ungureanu</div>
          <div style={{ fontSize: 12, color: "var(--color-brand-lavender)", fontWeight: 500, letterSpacing: "0.04em", textTransform: "uppercase" }}>Clarity & Direction Coach</div>
        </div>
      </div>
